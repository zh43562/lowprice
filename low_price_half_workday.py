...
import os
import time
import sys # 引入 sys 库，用于退出程序
import chinesecalendar # 引入我们强大的中国日历工具
...
# ###############################################################
today = date.today()
if not chinesecalendar.is_workday(today):
    print(f"{today.isoformat()} 不是中国A股交易日（周末或法定节假日），任务自动跳过。")
    sys.exit(0) # 正常退出，不执行任何操作
# ###############################################################


print(f"{today.isoformat()} 是交易日，自动化任务开始...")
...
df_final_2yuan = df_final_2yuan[['name_code2', '股票名称', '涨跌幅', '最新价', '最高', '最低', '今开', '涨跌额', '换手率', '量比', '动态市盈率', '成交量', '成交额', '总市值', '流通市值']]
...
df_final_cyb = df_final_cyb[['name_code2', '股票名称', '涨跌幅', '最新价', '最高', '最低', '今开', '涨跌额', '换手率', '量比', '动态市盈率', '成交量', '成交额', '总市值', '流通市值']]
...
send_email(
    subject=f"每日股市筛选结果 (智能跳过非交易日) - {today_str}",
    body="你好，\n\n请查收附件中的今日股市筛选结果。\n\n- Gemini Business",
